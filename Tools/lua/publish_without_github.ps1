...
function Backup-BinDirectory {
    ...
    if (Test-Path -Path $binPath) {
        ...
        if (Test-Path -Path $backupPath) {
            Write-Host "Папка binBackup уже существует. Пожалуйста, проверьте содержимое или удалите её вручную." -ForegroundColor Red
            exit
        }
        $maxRetries = 3
        $retryCount = 0
        ...
        while (-not $success -and $retryCount -lt $maxRetries) {
            try {
                Rename-Item -Path $binPath -NewName $backupPath -ErrorAction Stop
                $success = $true
            }
            catch {
                $retryCount++
                Write-Host "Ошибка при переименовании папки bin в binBackup. Попытка $retryCount из $maxRetries." -ForegroundColor Red
                Start-Sleep -Seconds 5
            }
        }
        if (-not $success) {
            Write-Host "Не удалось переименовать папку bin в binBackup после $maxRetries попыток. Скрипт остановлен." -ForegroundColor Red
            exit
        }
        Write-Host "Папка bin успешно переименована в binBackup." -ForegroundColor Yellow
    } 
    else {
