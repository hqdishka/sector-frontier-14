...
public sealed class ReflectSystem : EntitySystem
{
    ...
    private bool TryReflectProjectile(EntityUid user, EntityUid reflector, EntityUid projectile, ProjectileComponent? projectileComp = null, ReflectComponent? reflect = null)
    {
        ...
        {
            _adminLogger.Add(LogType.BulletHit, LogImpact.Low, $"{ToPrettyString(user)} reflected {ToPrettyString(projectile)} from {ToPrettyString(projectileComp.Weapon)} shot by {projectileComp.Shooter}");
        ...
        {
            _adminLogger.Add(LogType.BulletHit, LogImpact.Low, $"{ToPrettyString(user)} reflected {ToPrettyString(projectile)}");
        }
        ...
        [NotNullWhen(true)] out Vector2? newDirection)
    {
        ...
        if (shooter != null)
            _adminLogger.Add(LogType.HitScanHit, LogImpact.Low, $"{ToPrettyString(user)} reflected hitscan from {ToPrettyString(shotSource)} shot by {ToPrettyString(shooter.Value)}");
        else
            _adminLogger.Add(LogType.HitScanHit, LogImpact.Low, $"{ToPrettyString(user)} reflected hitscan from {ToPrettyString(shotSource)}");
